...
for ($i=0; $i < 10; $i++) {  // boucle pour savoir qu'elle sont les valeurs vides
    ...
    if ($infos[$i]){
        ...
        // erreur, renvoie vers la page index.php avec l'erreur de la variable i
        header("Location: inscription.php?erreur=".$i);
    }
}
...
if ($mdp != $confirm_mdp){
    ...
    header("Location: inscription.php?erreur=4"); // erreur, renvoie vers la page index.php avec l'erreur 4
...
if ($emailExist){ // si l'email n'est pas dans la base de données
    ...
    header("Location: inscription.php?erreur=2"); // email existe deja
